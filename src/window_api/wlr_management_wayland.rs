...
{
  Event as HandleEvent,
  State as HandleState,
...
impl WlrWindowManager
{
  pub async fn new() -> anyhow::Result< Self >
  {
    ...
    connection.event_queue.roundtrip( &mut toplevel_state ).unwrap();

    Ok ( Self { connection, toplevel_state, } )
  }
}
...
impl ActiveWindowManager for WlrWindowManager
{
  async fn get_active_window_data( &mut self ) -> Result< ActiveWindowData >
  {
    ...
    let active_window = self
      ...
      .ok_or( anyhow!( "Current window is not found by ID {active_window_id}" ) )?;
