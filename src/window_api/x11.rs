...
fn get_pid( conn : &Connection, window : Window, pid_atom : Atom ) -> Result< Option< u32 > >
{
  ...
  let result = conn.wait_for_reply( conn.send_request( &request ) )?;
...
fn get_active_window_atom( conn : &Connection ) -> Result< Atom >
{
  ...
  let active_window_atom = conn.wait_for_reply( conn.send_request( &request ) )?;
...
fn get_net_wm_name_atom( conn : &Connection ) -> Result< Atom >
{
  ...
  let response = conn.wait_for_reply( conn.send_request( &request ) )?;
  Ok( response.atom() )
}

pub fn get_name( conn : &Connection, window : Window, wm_name_atom : Atom ) -> Result< String >
{
  let request = &x::GetProperty
  {
    delete : false,
    window,
  ...
  let wm_name = conn.wait_for_reply( conn.send_request( request ) )?;
...
impl WindowData
{
  ...
  fn get_active_inner( &self ) -> Result< ActiveWindowData >
  {
    ...
    let process_name = super::process_name::get_process_name( process )?
...
{
  pub fn new() -> Result< Self > { Ok( Self{  data : None  } ) }
...
impl WindowManager for X11WindowManager
{
  ...
  fn get_idle_time( &mut self ) -> Result< u32 >
  {
    ...
    let idle = data.connection.send_request( &query_info );
