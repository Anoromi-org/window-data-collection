...
use windows::
{
  Win32::
  {
    ...
    System::
    {
      Diagnostics::Debug::{ FORMAT_MESSAGE_FROM_SYSTEM, FORMAT_MESSAGE_IGNORE_INSERTS, FormatMessageW },
      SystemInformation::GetTickCount64,
      SystemServices::{ LANG_ENGLISH, SUBLANG_ENGLISH_US },
      Threading::{ OpenProcess, PROCESS_NAME_WIN32, PROCESS_QUERY_INFORMATION, PROCESS_VM_READ, QueryFullProcessImageNameW },
    },
    UI::
    {
...
pub fn get_active() -> Result< ActiveWindowData >
{
  ...
  if id == 0
  {
    ...
    {
      let data = String::from_utf16( &message_buffer[0 .. size as usize] ).expect( "Failed to unwrap" );
      return Err( anyhow!( "Failed to get active window {data}" ) );
    }
  }
  let process_handle = unsafe { OpenProcess( PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, BOOL::from( false ), id ) }
  ...
  unsafe { CloseHandle( process_handle ) }.inspect_err( | e | error!( "Failed to close handle {e:?}" ) )?;
